...
const Menu: React.FC<MenuProps> = ({ categories }) => {
    ...
    const defaultExpandedIndex = categories.findIndex(
        (category) => category.title === state?.categoryTitle,
    );
    ...
    return (
        ...
        >
            <Box
                ...
                boxShadow={state?.categoryTitle === undefined ? 'none' : '0px 2px 4px  #0000000f'}
            >
                <Scrollbar
                    autoHide={false}
                    style={{ maxHeight: '100%' }}
                                        ...
                                        <AccordionPanel p='0'>
                                            <VStack gap='0' align='flex-start'>
                                                {category.sections.map((section, subIndex) => {
                                                    const isActive = state?.sectionName === section;
                                                    return (
                                                        <Link
                                                            as={RouterLink}
                                                            to='/vegan'
