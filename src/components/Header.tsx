...
const Header = () => {
    ...
    if (pathname === '/juicy') {
        crumbs.push({
            label: state?.sourceLabel || 'Самое сочное',
            to: '/juicy',
        });
    } else if (pathname === '/vegan') {
        crumbs.push({ label: 'Веганская кухня', to: '/vegan' });
        if (state?.sectionName) {
            crumbs.push({ label: state.sectionName, to: '/vegan' });
        }
    }
    ...
    return (
        <Box bg='#ffffd3' position='fixed' left='0' right='0' zIndex='5'>
            ...
            >
                <Flex align='center'>
                    <Flex align='flex-end'>
                        <Image src='./pot.svg' alt='logo' mr='6.97px' />
                        <Image
                            sx={{
